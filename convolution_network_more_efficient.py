...
'''
method modified to make more efficient:
bag of words total, bag of words per sample, padding 
how one calls th
'''
# -----------------------------------------------
	...
	for sent in sentences:
		...
		for word in sent:
			...
			try: temp[dic[word]]=1
				...
				pass
			sentence_matrix.append(np.array(temp))
...
# y_train = to_categorical(y_train, 9) # One-hot encode the labels
# print (y_train)
...
def bag_of_words_convolution_persample(data,region_size,stride,num_words,len_vocabulary):
	...
	window_height=math.ceil( float(padded_matrix.shape[0]-(region_size-1))/stride )
...
def bag_of_words_conversion(X_train,region_size,convolution_stride,words_in_sentence,len_vocabulary,stride):
	...
	window_height=math.ceil( float(padded_matrix_size-(region_size-1))/stride )
...
# more parallels
#len_vocabulary=data_preprocessor.get_vocab_len()
convoluted_input1=bag_of_words_conversion(X_train,region_size,convolution_stride,words,len_vocabulary,convolution_stride)
print (convoluted_input1)
